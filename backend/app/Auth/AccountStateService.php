...
final class AccountStateService
{
    ...
    /** @return array{active:bool, force_reset:bool, tenant_id:int, token_epoch:int}|null */
    public function forUser(int $userId): ?array
    {
        $db = $this->connection ?? Services::database();
        $row = $db->table('users u')
            ->select('u.active, u.deleted_at, u.tenant_id, u.token_epoch, COALESCE(i.force_reset, 0) AS force_reset', false)
        ...
        return [
            ...
            'token_epoch' => (int) ($row['token_epoch'] ?? 0),
        ];
    }
