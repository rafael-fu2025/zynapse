 ...
 *
 * Dispatch rules ported from the legacy module:
 *   1. Resolve the student by scan method (qr → qr_code,
 *      rfid → rfid_tag, manual → student_number). Unknown → 404.
 *   2. Duplicate guard: a non-duplicate check-in for the same student
 ...
 *      same convention as the three-strike counter.
 *   4. Otherwise a clinic encounter is opened (pending triage) and
 *      enqueued into today's walk-in queue under the same row-locked
...
final class CheckinService extends BaseService
{
    ...
    public function scan(array $input): array
    {
        ...
        return $this->txn(function () use ($input, $method, $stationId, $scannedAt, $userId): array {
            ...
            $column  = match ($method) {
                'qr'   => 'qr_code',
                'rfid' => 'rfid_tag',
                default => 'student_number',
            };
            $student = $this->db->table('patients_students')
                ->where($column, (string) $input['identifier'])
            ...
            $schoolId = (string) $student['student_number'];
            ...
            if ($dup !== null) {
                ...
                return $this->result($checkinId, 'duplicate', $student, 'Already checked in within the last 5 minutes.', null, null);
            }
            ...
            if ($appt !== null) {
                ...
                if ((string) $appt['status'] === 'scheduled') {
                    ...
                    return $this->result($checkinId, 'counselling_confirmed', $student, "Counselling booking {$window} confirmed.", (int) $appt['id'], null);
                }
                $checkinId = $this->insertCheckin($schoolId, $method, $stationId, 'counselling_already', (int) $appt['id'], null, $userId, $scannedAt);
                return $this->result($checkinId, 'counselling_already', $student, "Counselling appointment {$window} already checked in.", (int) $appt['id'], null);
            }

            // 4. Fallback: open a pending-triage encounter + queue it.
            $now = $this->utcNow();
            ...
            return $this->result($checkinId, 'clinic_queued', $student, "Added to the clinic queue at position {$position}.", null, ['encounter_id' => $encounterId, 'position' => $position]);
        });
    }
     ...
     * @return array<string, mixed>
     */
    private function result(int $checkinId, string $outcome, array $student, string $message, ?int $appointmentId, ?array $queue): array
    {
        // Severe-allergy alert (legacy hasSevereAllergy).
        ...
        return [
            ...
            'student' => [
                ...
                'year_level'     => $student['year_level'] !== null ? (int) $student['year_level'] : null,
            ],
            'allergy_alert' => $severe !== []
                ? 'SEVERE allergy: ' . implode(', ', array_map(static fn (array $a): string => (string) $a['allergen'], $severe))
                : null,
            'counselling_appointment_id' => $appointmentId,
            'queue' => $queue,
        ];
