...
function NowServingCard({ title, data, flash }: { title: string; data: QueueColumn; flash: boolean }) {
  ...
  return <article aria-label={`${title} now serving`} className={cn('flex h-40 min-h-40 max-h-40 min-w-0 flex-col overflow-hidden rounded-xl border bg-card p-4 text-center shadow-sm transition-colors duration-500 motion-reduce:transition-none', flash && 'border-primary bg-primary/10')}>
    ...
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center overflow-hidden">
      {active.length === 0 ? <p className="text-lg text-muted-foreground">No one in service.</p> : <div className="grid w-full min-h-0 gap-1 overflow-y-auto">{active.map((entry) => <div key={entry.queue_number} className="flex items-center justify-center gap-3 rounded-md bg-muted/40 px-2 py-1"><p aria-label={`Now serving ${entry.queue_number}`} className={cn('font-bold leading-none text-primary tabular-nums', active.length > 1 ? 'text-2xl' : 'text-5xl xl:text-6xl')}>{entry.queue_number}</p><div className="min-w-0 text-left">{entry.display_name !== undefined && <p className="truncate text-sm font-semibold">{entry.display_name}</p>}{entry.patient_school_id !== undefined && <p className="truncate tabular-nums text-xs text-muted-foreground">{entry.patient_school_id}</p>}</div></div>)}</div>}
    </div>
  </article>;
}
...
function WaitingColumn({ title, data, textSize, autoScroll, speed }: { title: string; data: QueueColumn; textSize: 'compact' | 'standard' | 'large'; autoScroll: boolean; speed: number }) {
  ...
  return <section aria-label={`${title} waiting`} className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
    <div className="mb-3 flex items-center justify-between gap-2 border-b pb-2"><h3 className="font-semibold">{title}</h3><span aria-label={`${data.waiting.length} waiting`} className="rounded-full bg-primary/10 px-2 py-0.5 tabular-nums text-sm font-bold text-primary">{data.waiting.length}</span></div>
    {data.waiting.length === 0 ? <p className="text-sm text-muted-foreground">No one waiting.</p> : <ul ref={listRef} className="min-h-0 flex-1 space-y-2 overflow-y-auto overflow-x-hidden pr-1 [scrollbar-color:hsl(var(--muted-foreground))_transparent] [scrollbar-width:thin]">
      {data.waiting.map((entry) => <li key={entry.queue_number} className="min-w-0 rounded-lg bg-muted/60 p-2.5">
        <p className={cn('font-bold text-primary tabular-nums', size)}>{entry.queue_number}</p>{entry.display_name !== undefined && <p className={cn('truncate font-medium', size)}>{entry.display_name}</p>}{entry.patient_school_id !== undefined && <p className="truncate tabular-nums text-xs text-muted-foreground">{entry.patient_school_id}</p>}
      </li>)}
    </ul>}
  </section>;
}
...
export default function QueueDisplayPage() {
  ...
  return <main className="flex min-h-dvh flex-col bg-background p-3 text-foreground lg:h-dvh lg:overflow-hidden lg:p-4">
    <header className="mb-3 flex shrink-0 items-center justify-between gap-4" aria-label="Queue display header">
      ...
      <time aria-label="Current date and time" className="text-right text-xs tabular-nums sm:text-sm lg:text-base">{formatClock(now)}</time>
    </header>
