/**
 * Sidebar — shadcn/ui composite sidebar.
 *
 * Provider-driven: cookie-persisted open state, Ctrl/Cmd+B shortcut,
 * mobile Sheet presentation, icon-collapse with tooltips. Styled by
 * the --sidebar-* design tokens in styles/index.css.
 *
 * forwardRef components (React 18); Tailwind v4 variable syntax.
 */
...
>(
  (
    ...
    // Internal state, used when the sidebar is uncontrolled.
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      {...props}
    >
      {/* Mobile opens a Sheet, so the hamburger ("snack bar") icon
          reads as the expected affordance. On desktop the trigger
          toggles the icon-rail, where PanelLeft stays the right
          metaphor. */}
      {isMobile ? <Menu aria-hidden /> : <PanelLeft aria-hidden />}
...
/**
 * Content area companion. Deviates from upstream (renders a <div>, not
 * <main>): SYNAPSE pages own their <main> landmark for the skip link.
 */
...
const sidebarMenuButtonVariants = cva(
  'peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground dark:data-[active=true]:bg-sidebar-primary dark:data-[active=true]:text-sidebar-primary-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0',
  {
    variants: {
      variant: {
